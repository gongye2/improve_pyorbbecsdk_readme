...
def get_firmware_path():
    ...
    while True:
        firmware_path = input(
            "Please input the path of the firmware file (.bin) to be updated (or 'q' to quit): "
        )
        ...
        firmware_path = (
            firmware_path.strip().strip("'").strip('"')
        ...
        try:
            ...
            if os.path.isfile(firmware_path):
                ...
                print(
                    "Invalid file format or path. Please provide a valid .bin file.\n"
                )
        except Exception as e:
...
def main():
    ...
    try:
        ...
        while True:
            ...
            try:
                devices[device_index].update_firmware(
                    firmware_path, firmware_update_callback, async_update=False
                )
                print(
                    "Firmware update completed successfully! Please reboot the device."
                )
            except Exception as e:
            ...
            if (
                input("Enter 'q' to quit, or any other key to continue: ").lower()
                == "q"
            ):
                break
