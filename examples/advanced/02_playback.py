...
def _collect_enabled_filters(device, undistortion_map=None):
...
def _load_sidecar_json(device, bag_path: str) -> bool:
...
def setup_camera(playback_device):
    ...
    #    This is used when no sidecar JSON exists or ApplicationConfig is
    #    unsupported / malformed.
    # ------------------------------------------------------------------
    if not use_app_config:
        sensors = [
...
def _resize_keep_aspect_ratio(img, target_w, target_h):
...
def create_display(width=1280, height=720):
    ...
    for idx, frame in enumerate(video_frames):
        ...
        if frame is not None:
            if total_elements == 1:
                display = _resize_keep_aspect_ratio(frame, width, height)
            else:
                resized = _resize_keep_aspect_ratio(frame, cell_w, cell_h)
    ...
    for i, (key, img) in enumerate(imu_frames.items()):
        ...
        y_start = row * cell_h
        if total_elements == 1:
            display = _resize_keep_aspect_ratio(img, width, height)
        else:
            resized = _resize_keep_aspect_ratio(img, cell_w, cell_h)
            display[y_start : y_start + cell_h, x_start : x_start + cell_w] = resized
...
def main():
    ...
    try:
        ...
        if preset_loaded:
            state.enabled_filters = _collect_enabled_filters(playback, state.undistortion_map)
            for stype, flist in state.enabled_filters.items():
                names = []
                for f in flist:
                    try:
