...
class SensorControlTest(unittest.TestCase):
    ...
    def turn_off_depth_auto_exposure(self):
        ...
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_DEPTH_AUTO_EXPOSURE_BOOL, False
        )
    ...
    def turn_on_depth_auto_exposure(self):
        if not self.device.is_property_supported(
            ...
            return
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_DEPTH_AUTO_EXPOSURE_BOOL, True
        )
    ...
    def turn_on_color_auto_exposure(self):
        if not self.device.is_property_supported(
            ...
            return
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_COLOR_AUTO_EXPOSURE_BOOL, True
        )
    ...
    def test_get_and_set_color_gain(self):
        ...
        curr_color_gain = self.device.get_int_property(
    ...
    def test_get_and_set_depth_gain(self):
        ...
        curr_depth_gain = self.device.get_int_property(
        ...
        new_depth_gain = self.device.get_int_property(
            OBPropertyID.OB_PROP_DEPTH_GAIN_INT
        )
        self.assertIsNotNone(new_depth_gain)
        self.assertEqual(new_depth_gain, depth_gain)
        self.device.set_int_property(
            OBPropertyID.OB_PROP_DEPTH_GAIN_INT, curr_depth_gain
        )
    ...
    def test_get_and_set_color_exposure(self):
        if not self.device.is_property_supported(
            ...
            return
        self.turn_off_color_auto_exposure()
        curr_color_exposure = self.device.get_int_property(
            OBPropertyID.OB_PROP_COLOR_EXPOSURE_INT
        )
        ...
        new_color_exposure = self.device.get_int_property(
            OBPropertyID.OB_PROP_COLOR_EXPOSURE_INT
        )
        self.assertIsNotNone(new_color_exposure)
        self.assertEqual(new_color_exposure, color_exposure)
        self.device.set_int_property(
            OBPropertyID.OB_PROP_COLOR_EXPOSURE_INT, curr_color_exposure
        )
        self.turn_on_color_auto_exposure()
    ...
    def test_get_and_set_ir_exposure(self):
        if not self.device.is_property_supported(
            ...
            return
        self.turn_off_ir_auto_exposure()
        curr_ir_exposure = self.device.get_int_property(
            OBPropertyID.OB_PROP_IR_EXPOSURE_INT
        )
        ...
        new_ir_exposure = self.device.get_int_property(
            OBPropertyID.OB_PROP_IR_EXPOSURE_INT
        )
        self.assertIsNotNone(new_ir_exposure)
        self.assertEqual(new_ir_exposure, ir_exposure)
        self.device.set_int_property(
            OBPropertyID.OB_PROP_IR_EXPOSURE_INT, curr_ir_exposure
        )
        self.turn_on_ir_auto_exposure()
    ...
    def test_get_and_set_color_auto_white_balance(self):
        ...
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_COLOR_AUTO_WHITE_BALANCE_BOOL, color_auto_white_balance
        )
    ...
    def test_get_and_set_soft_filter(self):
        if not self.device.is_property_supported(
            ...
            return
        curr_soft_filter = self.device.get_bool_property(
            OBPropertyID.OB_PROP_DEPTH_SOFT_FILTER_BOOL
        )
        ...
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_DEPTH_SOFT_FILTER_BOOL, soft_filter
        )
        new_soft_filter = self.device.get_bool_property(
            OBPropertyID.OB_PROP_DEPTH_SOFT_FILTER_BOOL
        )
        self.assertIsNotNone(new_soft_filter)
        self.assertEqual(new_soft_filter, soft_filter)
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_DEPTH_SOFT_FILTER_BOOL, curr_soft_filter
        )
    ...
    def test_get_and_set_color_mirror(self):
        if not self.device.is_property_supported(
            ...
            return
        curr_color_mirror = self.device.get_bool_property(
            OBPropertyID.OB_PROP_COLOR_MIRROR_BOOL
        )
        ...
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_COLOR_MIRROR_BOOL, color_mirror
        )
        new_color_mirror = self.device.get_bool_property(
            OBPropertyID.OB_PROP_COLOR_MIRROR_BOOL
        )
        self.assertIsNotNone(new_color_mirror)
        self.assertEqual(new_color_mirror, color_mirror)
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_COLOR_MIRROR_BOOL, curr_color_mirror
        )
    ...
    def test_get_and_set_depth_mirror(self):
        if not self.device.is_property_supported(
            ...
            return
        curr_depth_mirror = self.device.get_bool_property(
            OBPropertyID.OB_PROP_DEPTH_MIRROR_BOOL
        )
        ...
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_DEPTH_MIRROR_BOOL, depth_mirror
        )
        new_depth_mirror = self.device.get_bool_property(
            OBPropertyID.OB_PROP_DEPTH_MIRROR_BOOL
        )
        self.assertIsNotNone(new_depth_mirror)
        self.assertEqual(new_depth_mirror, depth_mirror)
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_DEPTH_MIRROR_BOOL, curr_depth_mirror
        )
    ...
    def test_get_and_set_ir_mirror(self):
        if not self.device.is_property_supported(
            ...
            return
        curr_ir_mirror = self.device.get_bool_property(
            OBPropertyID.OB_PROP_IR_MIRROR_BOOL
        )
        ...
        new_ir_mirror = self.device.get_bool_property(
            OBPropertyID.OB_PROP_IR_MIRROR_BOOL
        )
        self.assertIsNotNone(new_ir_mirror)
        self.assertEqual(new_ir_mirror, ir_mirror)
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_IR_MIRROR_BOOL, curr_ir_mirror
        )
    ...
    def test_get_and_set_color_auto_exposure(self):
        if not self.device.is_property_supported(
            ...
            print("Current device not support color auto exposure!")
            return
        curr_color_auto_exposure = self.device.get_bool_property(
            OBPropertyID.OB_PROP_COLOR_AUTO_EXPOSURE_BOOL
        )
        ...
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_COLOR_AUTO_EXPOSURE_BOOL, color_auto_exposure
        )
        new_color_auto_exposure = self.device.get_bool_property(
            OBPropertyID.OB_PROP_COLOR_AUTO_EXPOSURE_BOOL
        )
        self.assertIsNotNone(new_color_auto_exposure)
        self.assertEqual(new_color_auto_exposure, color_auto_exposure)
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_COLOR_AUTO_EXPOSURE_BOOL, curr_color_auto_exposure
        )
    ...
    def test_get_and_set_depth_auto_exposure(self):
        if not self.device.is_property_supported(
            ...
            print("Current device not support depth auto exposure!")
            return
        curr_depth_auto_exposure = self.device.get_bool_property(
            OBPropertyID.OB_PROP_DEPTH_AUTO_EXPOSURE_BOOL
        )
        ...
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_DEPTH_AUTO_EXPOSURE_BOOL, depth_auto_exposure
        )
        new_depth_auto_exposure = self.device.get_bool_property(
            OBPropertyID.OB_PROP_DEPTH_AUTO_EXPOSURE_BOOL
        )
        self.assertIsNotNone(new_depth_auto_exposure)
        self.assertEqual(new_depth_auto_exposure, depth_auto_exposure)
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_DEPTH_AUTO_EXPOSURE_BOOL, curr_depth_auto_exposure
        )
    ...
    def test_get_and_set_ir_auto_exposure(self):
        if not self.device.is_property_supported(
            ...
            print("Current device not support ir auto exposure!")
            return
        curr_ir_auto_exposure = self.device.get_bool_property(
            OBPropertyID.OB_PROP_IR_AUTO_EXPOSURE_BOOL
        )
        ...
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_IR_AUTO_EXPOSURE_BOOL, ir_auto_exposure
        )
        new_ir_auto_exposure = self.device.get_bool_property(
            OBPropertyID.OB_PROP_IR_AUTO_EXPOSURE_BOOL
        )
        self.assertIsNotNone(new_ir_auto_exposure)
        self.assertEqual(new_ir_auto_exposure, ir_auto_exposure)
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_IR_AUTO_EXPOSURE_BOOL, curr_ir_auto_exposure
        )
    ...
    def test_get_and_set_color_flip(self):
        if not self.device.is_property_supported(
            ...
            return
        curr_color_flip = self.device.get_bool_property(
            OBPropertyID.OB_PROP_COLOR_FLIP_BOOL
        )
        ...
        new_color_flip = self.device.get_bool_property(
            OBPropertyID.OB_PROP_COLOR_FLIP_BOOL
        )
        self.assertIsNotNone(new_color_flip)
        self.assertEqual(new_color_flip, color_flip)
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_COLOR_FLIP_BOOL, curr_color_flip
        )
    ...
    def test_get_and_set_depth_flip(self):
        if not self.device.is_property_supported(
            ...
            return
        curr_depth_flip = self.device.get_bool_property(
            OBPropertyID.OB_PROP_DEPTH_FLIP_BOOL
        )
        ...
        new_depth_flip = self.device.get_bool_property(
            OBPropertyID.OB_PROP_DEPTH_FLIP_BOOL
        )
        self.assertIsNotNone(new_depth_flip)
        self.assertEqual(new_depth_flip, depth_flip)
        self.device.set_bool_property(
            OBPropertyID.OB_PROP_DEPTH_FLIP_BOOL, curr_depth_flip
        )
    ...
    def test_get_and_set_color_sharpness(self):
        if not self.device.is_property_supported(
            ...
            return
        self.turn_off_color_auto_exposure()
        curr_color_sharpness = self.device.get_int_property(
            OBPropertyID.OB_PROP_COLOR_SHARPNESS_INT
        )
        ...
        self.device.set_int_property(
            OBPropertyID.OB_PROP_COLOR_SHARPNESS_INT, color_sharpness
        )
        new_color_sharpness = self.device.get_int_property(
            OBPropertyID.OB_PROP_COLOR_SHARPNESS_INT
        )
        self.assertIsNotNone(new_color_sharpness)
        self.assertEqual(new_color_sharpness, color_sharpness)
        self.device.set_int_property(
            OBPropertyID.OB_PROP_COLOR_SHARPNESS_INT, curr_color_sharpness
        )
        self.turn_on_color_auto_exposure()
    ...
    def test_get_and_set_color_hue(self):
        if not self.device.is_property_supported(
            ...
            return
        self.turn_off_color_auto_exposure()
        curr_color_hue = self.device.get_int_property(
            OBPropertyID.OB_PROP_COLOR_HUE_INT
        )
    ...
    def test_get_and_set_color_contrast(self):
        if not self.device.is_property_supported(
            ...
            return
        self.turn_off_color_auto_exposure()
        curr_color_contrast = self.device.get_int_property(
            OBPropertyID.OB_PROP_COLOR_CONTRAST_INT
        )
        ...
        self.device.set_int_property(
            OBPropertyID.OB_PROP_COLOR_CONTRAST_INT, color_contrast
        )
        new_color_contrast = self.device.get_int_property(
            OBPropertyID.OB_PROP_COLOR_CONTRAST_INT
        )
        self.assertIsNotNone(new_color_contrast)
        self.assertEqual(new_color_contrast, color_contrast)
        self.device.set_int_property(
            OBPropertyID.OB_PROP_COLOR_CONTRAST_INT, curr_color_contrast
        )
        self.turn_on_color_auto_exposure()
    ...
    def test_get_and_set_color_brightness(self):
        if not self.device.is_property_supported(
            ...
            return
        self.turn_off_color_auto_exposure()
        curr_color_brightness = self.device.get_int_property(
            OBPropertyID.OB_PROP_COLOR_BRIGHTNESS_INT
        )
        ...
        self.device.set_int_property(
            OBPropertyID.OB_PROP_COLOR_BRIGHTNESS_INT, color_brightness
        )
        new_color_brightness = self.device.get_int_property(
            OBPropertyID.OB_PROP_COLOR_BRIGHTNESS_INT
        )
        self.assertIsNotNone(new_color_brightness)
        self.assertEqual(new_color_brightness, color_brightness)
        self.device.set_int_property(
            OBPropertyID.OB_PROP_COLOR_BRIGHTNESS_INT, curr_color_brightness
        )
        self.turn_on_color_auto_exposure()
    ...
    def test_get_and_set_fan_work_mode(self):
        if not self.device.is_property_supported(
            ...
            return
        curr_fan_work_mode = self.device.get_int_property(
            OBPropertyID.OB_PROP_FAN_WORK_MODE_INT
        )
        ...
        self.device.set_int_property(
            OBPropertyID.OB_PROP_FAN_WORK_MODE_INT, fan_work_mode
        )
        new_fan_work_mode = self.device.get_int_property(
            OBPropertyID.OB_PROP_FAN_WORK_MODE_INT
        )
        self.assertIsNotNone(new_fan_work_mode)
        self.assertEqual(new_fan_work_mode, fan_work_mode)
        self.device.set_int_property(
            OBPropertyID.OB_PROP_FAN_WORK_MODE_INT, curr_fan_work_mode
        )


if __name__ == "__main__":
    print(
        "Start test SensorControl interface, Please make sure you have connected a device to your computer."
    )
    unittest.main()
